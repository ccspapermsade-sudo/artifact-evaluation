#!/usr/bin/env python3
"""
Simple heuristic baseline for the camera-first RL environment.

Heuristic:
- fixed size action (small/medium)
- zero rotation
- zero jitter x
- zero jitter y
- environment cursor/grid handles fill order (near region first)
"""
...
def parse_args():
    ...
    p.add_argument("--size_action", type=float, default=0.30,
                   help="Fixed size action in [0,1]. 0.30 is small/medium.")
    p.add_argument("--rot_action", type=float, default=0.50,
                   help="Fixed rotation action in [0,1]. Usually 0.50 means zero rotation.")
    p.add_argument("--jx_action", type=float, default=0.50,
                   help="Fixed jitter-x action in [0,1]. Usually 0.50 means zero jitter.")
    p.add_argument("--jy_action", type=float, default=0.50,
                   help="Fixed jitter-y action in [0,1]. Usually 0.50 means zero jitter.")
...
def main():
    ...
    img_dir = out_dir / "heuristic_rollouts"
    img_dir.mkdir(parents=True, exist_ok=True)

    cfg = vars(args).copy()
    with open(out_dir / "heuristic_baseline_config.json", "w") as f:
        json.dump(cfg, f, indent=2)

    csv_path = out_dir / "heuristic_baseline_results.csv"
    rows = []

    heuristic_action = np.array([
        clamp01(args.size_action),
        clamp01(args.rot_action),
        clamp01(args.jx_action),
        clamp01(args.jy_action),
    ], dtype=np.float32)

    print("Using fixed heuristic action:", heuristic_action.tolist())
    ...
    for ep in range(1, args.episodes + 1):
        ...
        while not done:
            obs, reward, terminated, truncated, info = env.step(heuristic_action)
        ...
        row = {
            ...
            "size_action": float(heuristic_action[0]),
            "rot_action": float(heuristic_action[1]),
            "jx_action": float(heuristic_action[2]),
            "jy_action": float(heuristic_action[3]),
        }
        rows.append(row)

        print(
            f"[Heuristic ep {ep:03d}] "
        ...
        if args.save_every > 0 and (ep % args.save_every == 0):
            save_rollout_images(env, img_dir, f"ep{ep:03d}")
    ...
    with open(out_dir / "heuristic_baseline_summary.json", "w") as f:
