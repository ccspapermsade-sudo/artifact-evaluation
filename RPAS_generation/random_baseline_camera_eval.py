#!/usr/bin/env python3
"""
Random baseline for the camera-first RL environment.

It uses the SAME environment and SAME arguments as the RL code, but instead of PPO:
- at each step, samples action uniformly from [0,1]^4
- runs multiple episodes
- saves CAMERA / PLANE rollout images
- writes per-episode results to CSV
"""
...
def parse_args():
    ...
    p.add_argument("--episodes", type=int, default=20, help="Number of random episodes")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--eval_mode", action="store_true", help="Use budget_fill_cap like RL export_rollouts")
    p.add_argument("--save_every", type=int, default=1, help="Save rollout images every N episodes")
    p.add_argument("--out_dir", type=str, required=True)

    return p.parse_args()


def save_rollout_images(env, save_dir: Path, prefix: str, precompensate_gamma: float):
...
def main():
    ...
    img_dir = out_dir / "random_rollouts"
    img_dir.mkdir(parents=True, exist_ok=True)

    cfg = vars(args).copy()
    if hasattr(rl_mod, "H_CONST"):
        try:
            cfg["H_CONST"] = rl_mod.H_CONST.tolist()
        except Exception:
            pass
    with open(out_dir / "random_baseline_config.json", "w") as f:
        json.dump(cfg, f, indent=2)

    csv_path = out_dir / "random_baseline_results.csv"
    rows = []
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if args.save_every > 0 and (ep % args.save_every == 0):
            save_rollout_images(
                ...
                save_dir=img_dir,
                prefix=f"ep{ep:03d}",
                precompensate_gamma=args.precompensate_gamma,
            )
    ...
    summary = {
        ...
        "best_post_mean": float(best_posts.mean()) if len(best_posts) else 0.0,
        "best_post_std": float(best_posts.std()) if len(best_posts) else 0.0,
        "best_post_max": float(best_posts.max()) if len(best_posts) else 0.0,
        "reward_mean": float(rewards.mean()) if len(rewards) else 0.0,
        "reward_std": float(rewards.std()) if len(rewards) else 0.0,
        "placed_mean": float(placeds.mean()) if len(placeds) else 0.0,
    ...
    with open(out_dir / "random_baseline_summary.json", "w") as f:
